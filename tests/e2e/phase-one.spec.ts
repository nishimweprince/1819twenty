...
const publicRoutes = ["/", "/about", "/designers", "/designers/apply", "/privacy", "/terms"];
...
test("primary navigation exposes only Phase 1 destinations", async ({ page }) => {
  await page.goto("/");
...
test("designer form keeps the visitor on the first step when required fields are absent", async ({ page }) => {
  await page.goto("/designers/apply");
  await page.getByRole("button", { name: "Continue" }).click();
  await expect(page.getByText("Enter the brand or designer name.")).toBeVisible();
