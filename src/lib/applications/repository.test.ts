...
beforeEach(() => {
  ...
  listMock.mockResolvedValue({
    data: paths.map((path) => ({ name: path.split("/")[1] })),
    error: null,
  });
});
...
describe("verifyUploads", () => {
  ...
  it("rejects missing, duplicate, foreign, or nested paths", async () => {
    expect(
      await verifyUploads(id, [...paths.slice(0, 2), `${id}/missing.jpg`]),
    ).toBe(false);
    expect(await verifyUploads(id, [paths[0], paths[0], paths[1]])).toBe(false);
    expect(
      await verifyUploads(id, [paths[0], paths[1], `another-id/work-3.jpg`]),
    ).toBe(false);
    expect(
      await verifyUploads(id, [paths[0], paths[1], `${id}/nested/work-3.jpg`]),
    ).toBe(false);
  });
});
