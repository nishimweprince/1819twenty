...
async function fillThroughReview() {
  ...
  fireEvent.change(screen.getByLabelText(/^Full name/i), {
    target: { value: "Aline Example" },
  });
  fireEvent.change(screen.getByLabelText(/^Brand\/business name/i), {
    target: { value: "Kigali Studio" },
  });
  fireEvent.change(screen.getByLabelText(/^Email address/i), {
    target: { value: "aline@example.com" },
  });
  fireEvent.change(screen.getByLabelText(/^Phone number/i), {
    target: { value: "+250780000000" },
  });
  fireEvent.change(screen.getByLabelText(/^Country\/city based in/i), {
    target: { value: "Kigali, Rwanda" },
  });
  fireEvent.change(screen.getByLabelText(/^Instagram\/social handles/i), {
    target: { value: "@kigalistudio" },
  });
  ...
  fireEvent.change(screen.getByLabelText(/^9\. Tell us about/i), {
    target: {
      value:
        "We create each collection with local cloth and a small team of makers.",
    },
  });
  choose(/^10\. How long/i, "3–5 years");
  choose(/^11\. Who/i, "A small team");
  fireEvent.change(screen.getByLabelText(/^11\. Where/i), {
    target: { value: "Kigali, Rwanda" },
  });
  ...
  fireEvent.click(
    within(
      screen.getByRole("group", { name: /12\. Do you currently sell online/i }),
    ).getByLabelText("Yes"),
  );
  fireEvent.change(screen.getByLabelText(/^12\. If so/i), {
    target: { value: "Instagram" },
  });
  choose(/^13\. What's/i, "20–50 pieces");
  fireEvent.click(
    within(
      screen.getByRole("group", { name: /14\. Do you have existing/i }),
    ).getByLabelText("No"),
  );
  ...
  const files = [1, 2, 3].map(
    (index) => new File(["photo"], `work-${index}.png`, { type: "image/png" }),
  );
  fireEvent.change(screen.getByLabelText(/^16\. Upload/i), {
    target: { files },
  });
  expect(
    screen.getByRole("list", { name: "Selected photos" }).children,
  ).toHaveLength(3);
  await continueTo("Fit & goals");

  fireEvent.change(screen.getByLabelText(/^18\. Why/i), {
    target: {
      value:
        "We want to introduce our work and our makers to a wider audience.",
    },
  });
  fireEvent.click(screen.getByLabelText("Global reach"));
  fireEvent.change(screen.getByLabelText(/^20\. Anything/i), {
    target: { value: "" },
  });
