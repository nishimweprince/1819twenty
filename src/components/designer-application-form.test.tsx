...
async function completeStepZero() {
  fireEvent.change(screen.getByLabelText(/Brand or designer name/i), { target: { value: "Test Brand" } });
  fireEvent.change(screen.getByLabelText(/Contact name/i), { target: { value: "Test Contact" } });
  fireEvent.change(screen.getByLabelText(/Email address/i), { target: { value: "test@example.com" } });
  fireEvent.change(screen.getByLabelText(/Phone or WhatsApp/i), { target: { value: "+250788000000" } });
  fireEvent.change(screen.getByLabelText(/Country and city/i), { target: { value: "Kigali, Rwanda" } });
  fireEvent.change(screen.getByLabelText(/Website or social handle/i), { target: { value: "@testbrand" } });
...
async function completeStepOne() {
  ...
  fireEvent.change(screen.getByLabelText(/Available items or SKUs/i), { target: { value: "12" } });
...
async function completeStepTwo() {
  ...
  fireEvent.change(screen.getByLabelText(/Brand story or description/i), {
    target: { value: "A maker story long enough to satisfy the forty character minimum." },
  });
...
describe("DesignerApplicationForm without provider env", () => {
  it("simulates the send and flags the confirmation as simulated", async () => {
    ...
    expect(destination).toMatch(/^\/designers\/apply\/received\?reference=SIM-[0-9A-F]{8}&simulated=1$/);
  });
});
