...
export async function POST(request: Request) {
  if (!env.RESEND_WEBHOOK_SECRET) return failure("NOT_CONFIGURED", "Webhook verification is not configured.", 503);
  ...
  try {
    ...
    if (emailId) await updateDeliveryByEmailId(emailId, event.type.replace("email.", ""));
