    ...
    <>
      ...
      >
        ...
        <div className="container relative z-1 flex flex-col items-center text-center">
          ...
          <h1
            id="site-title"
            className="mb-4 max-w-[20ch] text-[clamp(2.2rem,4.2vw,3.8rem)] text-paper"
          >
            Eighteen Nineteen Twenty
          </h1>
          <p
            className={`${ceremonial} mb-8 max-w-[44ch] leading-[1.9] text-paper`}
          >
      ...
      <section
        className="border-y border-ink/12 bg-paper-hi py-9"
      ...
      >
        <div className="container grid grid-cols-3 gap-px bg-ink/12 max-[700px]:grid-cols-1">
          {pillars.map(([icon, title, copy], index) => (
            <div
              className="flex items-center gap-5 bg-paper-hi px-7 py-3 max-[700px]:px-4"
              key={title}
            >
              <BrandIcon
                name={icon}
                className={`size-11 flex-none ${index % 2 ? "text-gold" : "text-terracotta"}`}
              />
              <div>
      ...
      <section
        className="py-20 max-[700px]:py-16"
        aria-labelledby="collection-title"
      >
        <div className="container">
          <div className="mb-8 flex items-end justify-between gap-4">
            <h2 id="collection-title" className="mb-0">
              ...
              >
                <figure className="m-0">
                  ...
                  <figcaption className="pt-3">
                    ...
                    <span className="mt-1 block text-[0.85rem] text-ink/75">
                      Coming soon
                    </span>
                  </figcaption>
                </figure>
      ...
      <section
        className="border-y border-ink/12 bg-paper-hi py-16 max-[700px]:py-12"
        aria-labelledby="join-title"
      >
        <div className="narrow flex flex-col items-center text-center">
          <h2 id="join-title" className="mb-3">
            Join Our Community
          </h2>
      ...
      <section
        className="grid min-h-96 grid-cols-[1fr_1.2fr_1fr] border-t border-ink/12 max-[900px]:grid-cols-1"
        aria-labelledby="more-than-name"
      >
        <div className={`${zoomFrame} min-h-96 max-[900px]:min-h-64`}>
          <Image
            ...
            fill
            className={`${zoomImage} object-[center_40%]`}
            sizes="(max-width: 900px) 100vw, 33vw"
          />
          {/* Ink script on the bright sky, which sits top right of this crop. */}
          <span
            className={`${scriptAccent} absolute right-7 top-7 max-w-[7ch] text-center text-ink`}
            aria-hidden="true"
          >
        ...
        <div className="flex flex-col items-start justify-center bg-paper-hi px-[clamp(2rem,5vw,5rem)] py-16">
          <h2 id="more-than-name" className="mb-4">
            More Than a Name
          </h2>
          ...
          <Link href="/about" className={`${button} mt-2`}>
            Read our story
          </Link>
        </div>
        <div className={`${zoomFrame} min-h-96 max-[900px]:min-h-64`}>
          <Image
            ...
            alt="Indigo tie-dyed cloth with a white diamond grid"
            fill
            className={zoomImage}
            sizes="(max-width: 900px) 100vw, 33vw"
          />
          <div
            aria-hidden="true"
            className="absolute inset-y-0 right-0 w-1/2 bg-linear-to-l from-ink/45 to-transparent"
          />
          <p
            className={`${ceremonial} absolute bottom-8 right-7 mb-0 grid justify-items-end gap-1.5 text-right leading-[1.9] text-paper`}
            aria-hidden="true"
          >
            <span>People</span>
            <span>Places</span>
